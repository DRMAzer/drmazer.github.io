...
API_TOKEN = '8211772439:AAEypS2JO_htIGjde08xyTLZowVTXjmLg4c'
ADMIN_ID = 8574641551 
...
GITHUB_TOKEN = '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'
...
PORT_DETAILS = {
    "13021": {"loc": "USA/TEXAS", "rotation": 30, "host": "interchange.proxy.rlwy.net"},
    "13813": {"loc": "USA/FLORIDA", "rotation": 30, "host": "shuttle.proxy.rlwy.net"},
    "23822": {"loc": "USA/CALIFORNIA", "rotation": 30, "host": "switchback.proxy.rlwy.net"},
    "33451": {"loc": "USA/NEW YORK", "rotation": 30, "host": "ballast.proxy.rlwy.net"},
    "42177": {"loc": "USA/GEORGIA", "rotation": 30, "host": "maglev.proxy.rlwy.net"}
}
...
def save_data():
    ...
    # ملاحظة: سيتم دمج جميع المستخدمين تحت بورت 8080
    for uid, proxies in active_proxies.items():
        for proxy in proxies:
            cfg_content += f"users {proxy['user']}:CL:{proxy['pass']}\n"
            cfg_content += f"allow {proxy['user']}\n"

    # تشغيل البروكسي على بورت 8080 (التوافق مع Railway)
    # نستخدم الأي بي الخارجي لباقة AT&T التي زودتني بها
    cfg_content += "\nproxy -p8080 -a -n -R -E95.169.180.49 -EXTIP95.169.180.49:8496\n"
    cfg_content += "socks -p8080 -a -n -R -E95.169.180.49 -EXTIP95.169.180.49:8496\n"

    # كود رفع الملف إلى GitHub (تأكد من وجود متغيرات GitHub لديك)
    try:
        url = f"https://api.github.com/repos/{REPO_PATH}/contents/3proxy.cfg"
        res = requests.get(url, headers={"Authorization": f"token {GITHUB_TOKEN}"})
        sha = res.json().get('sha') if res.status_code == 200 else None
        
        payload = {
            "message": "Update proxy config to port 8080",
        ...
        print("✅ تم تحديث السيرفر بنجاح على بورت 8080")
        ...
        try:
            ...
            for uid in list(active_proxies.keys()):
                ...
                for sub in active_proxies[uid]:
                    ...
                    if remaining_seconds > 0: updated_subs.append(sub)
                    else: changed = True
                active_proxies[uid] = updated_subs
                if not active_proxies[uid]: del active_proxies[uid]
